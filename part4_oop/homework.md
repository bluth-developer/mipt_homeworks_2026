...
Система должна поддерживать "тихую" замену компонентов. Например, если мы заменим `FIFOPolicy` на `LRUPolicy` в конструкторе `Cache`, логика работы самого кэша не должна измениться. Это демонстрирует принципы **Liskov Substitution** (L) и **Dependency Inversion** (D).
